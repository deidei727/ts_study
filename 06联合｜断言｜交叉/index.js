...
let aaa = (x) => {
    ...
    // x.run
    // 方式一
    x.run; //使用断言来推断传入的是接口A
    // 方式二
    // (<A>x).run
    // 不能将一个变量断言成两个东西
};
console.log(aaa({ build: 24 }));
// 但是我们要注意，断言只是欺骗编译器，无法避免运行时的错误，在编译过程中会删除类型断言，不要滥用类型断言，否则会导致运行错误
...
// 对字面量的断言
// as const
// 如果是普通类型，和const定义常量是一样的，但是引用类型是不一样的
...
let arr10 = [1, 2, 3, 4, 5, 6, 7, 8]; //此时数据已经断言成[1,2,3,4,5,6,7,8]，他并不是一个数组，所以下面使用数组方法对时候会报错，说没有此属性
arr0.push(7); //正确
// arr10.push(7)
